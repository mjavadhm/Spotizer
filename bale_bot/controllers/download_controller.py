...
class BaleDownloadController:
                ...
                if make_zip:
                    if content_type == 'artist':
                        ...
                        if all_success:
                            ...
                            # We don't want to fallback to sending 100s of tracks individually for discography
                            # as it would spam the user. Just stop here.
                            pass
                    else:
    ...
    async def process_artist_discography(self, user_id: int, artist_id: str):
        ...
        try:
            ...
            # Get user settings
            async with bale_async_session_maker() as session:
                from bale_bot.database.models import BaleUserSettings
                    ...
                    for root, _, files in os.walk(chunk_dir):
                        for file in files:
                            if file.lower().endswith(('.mp3', '.flac', '.m4a')):
                                ...
                                document = bale.InputFile(file_path)
                                try:
